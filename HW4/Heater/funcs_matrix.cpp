...
double matrixType::f(double x, double y,
                double A,
                double x0, double y0,
                double sigX, double sigY)
{
    double ex = (x - x0) * (x - x0) / (2.0 * sigX * sigX);
    double ey = (y - y0) * (y - y0) / (2.0 * sigY * sigY);
    return (A * std::exp(-(ex + ey)));
}
	...
	for (int i = 1; i <= Nx; i++)
	{
		for (int j = 1; j <= Ny; j++)
		{
			...
			double src = f(x, y,
			               BC.Ts,
			               BC.h1, BC.h2,
			               0.02*Mesh.Lx, 0.02*Mesh.Ly);

			setRHS(i,j,dx*dy*src);
		}
	}
