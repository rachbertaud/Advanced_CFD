...
Err_inf = [];
Dvals = {'1e-9', '1e-10', '1e-11', '1e-12'};
for i = 1:length(Dvals)
    N = Dvals{i};
    x = readmatrix(['xc_D' N '.dat']);
    y = readmatrix(['yc_D' N '.dat']);
    c_num = readmatrix(['c_D' N '.dat']);
    ...
    colorbar
    shading flat
    % name = "HW5_ExtrapBC_D" + N +".png";
    % saveas(f, name, 'png')
    
    f = figure(2);
    theme(f, "light")
    plot(X_NB(1,:), c_num_NB(1,:), color=[227/255,159/255,246/255])
    hold on 
    ylabel("$$C(x,0)$$")
    xlabel("$$x$$")
    % plot(x, c_num(1,:), 'm')
    % name = "HW5_ExtrapBC_D" + N + "_Edge.png";
    % saveas(f, name, 'png')
